...
class Utils:
    ...
    @staticmethod
    def convert_cv_qt(cv_img, images_dict = None):
        """Convert from an opencv image to QPixmap"""
        rgb_image = cv2.cvtColor(cv_img, cv2.COLOR_BGR2RGB)
        h, w, ch = rgb_image.shape
        bytes_per_line = ch * w
        convert_to_Qt_format = QtGui.QImage(rgb_image.data, w, h, bytes_per_line, QtGui.QImage.Format_RGB888)
        p = convert_to_Qt_format.scaled(Config.display_width, Config.display_height, Qt.KeepAspectRatio)
        result = QtGui.QPixmap.fromImage(p)
        if(images_dict): # set image size
            images_dict["image_size"] = {"width":result.width(), "height": result.height()}
        return result
